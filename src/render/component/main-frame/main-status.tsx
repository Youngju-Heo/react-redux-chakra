...
const MainStatus = (props: MainStatusProps): JSX.Element => {
  ...
  const showMessage = `[${FormatNumberArray(view?.center || [0, 0], 6)}, ${FormatNumber(view?.zoom, 4)}] ${message}`;

  React.useEffect(() => {
    if (isElectron()) {
      console.log("is-electron");
      window.electronApi.setMessage(showMessage);
    }
  }, [showMessage]);

  return (
    <Flex p={2} h={10} w="100%" bg="rgba(13,16,31,.3)">
      <Center>{showMessage}</Center>
      <Spacer />
      <Center>
        <TimeWidget />
      </Center>
